...
const TITLE_BY_PATH = {
  "/admin/lucky-spin": "Lucky Spin Management",
  "/admin/lucky-spin/prize-settings": "Prize Settings",
  "/admin/lucky-spin/user-logs": "User Logs",
  "/admin/lucky-spin/daily-limits": "Daily Limits",
};

function HeaderActions() {
  // The decorative gold-dot button that exists on every sub-page header.
  return (
    <button type="button" className="flex h-[26px] w-[26px] items-center justify-center" aria-label="Page actions">
...
export default function LuckySpinLayout({ children }) {
  const pathname = usePathname();
  const title = TITLE_BY_PATH[pathname] ?? "Lucky Spin Management";

  return (
    <AdminRouteGuard>
      <main className="min-h-screen xl:admin-content-pl pr-10 pt-10 pb-10">
        <div className="mb-8 flex items-start justify-between">
          <h1 className="text-4xl font-bold leading-[1.05] text-white">{title}</h1>
          <HeaderActions />
        </div>
