...
import {
  getWithdrawalBanner,
  createWithdrawalBanner,
  updateWithdrawalBanner,
} from "../../../../api/adminApi";
...
function BannerForm() {
  ...
  useEffect(() => {
    ...
    getWithdrawalBanner(editingUuid)
      .then((b) => {
        setForm({
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editingUuid) {
        await updateWithdrawalBanner(editingUuid, payload);
      } else {
        await createWithdrawalBanner(payload);
      }
...
export default function AddWithdrawalBannerPage() {
  return (
