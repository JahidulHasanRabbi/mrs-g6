"use client";

import { Suspense, useEffect, useState } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import FormChrome, { INPUT_BASE } from "../../../../components/admin/world-cup/FormChrome";
import {
  getWithdrawalDummyPlayer,
  createWithdrawalDummyPlayer,
  updateWithdrawalDummyPlayer,
} from "../../../../api/adminApi";
import { MOCK_WITHDRAWAL_PLAYERS, isMockUuid } from "../../../../components/admin/leaderboards/mockData";

function DummyForm() {
  const router = useRouter();
  const params = useSearchParams();
  const editingUuid = params.get("uuid");

  const [form, setForm] = useState({
    name: "",
    totalWithdraw: "",
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    if (!editingUuid) return;
    const applyMock = () => {
      const m = MOCK_WITHDRAWAL_PLAYERS.find((x) => x.uuid === editingUuid);
      if (m) {
        setForm({
          name: m.player_name ?? "",
          totalWithdraw: String(m.total_withdrawal ?? ""),
        });
      }
    };
    if (isMockUuid(editingUuid)) {
      applyMock();
      return;
    }
    getWithdrawalDummyPlayer(editingUuid)
      .then((d) => {
        setForm({
          name: d.player_name ?? "",
          totalWithdraw: String(d.total_withdrawal ?? d.total_withdraw ?? ""),
        });
      })
      .catch(applyMock);
  }, [editingUuid]);

  const set = (k) => (v) =>
    setForm((p) => ({ ...p, [k]: typeof v === "object" && v?.target ? v.target.value : v }));

  const toNum = (s) => Number(String(s).replace(/[,\s#]/g, "")) || 0;

  const onSave = async () => {
    if (!form.name) {
      setError("Player name is required.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      const payload = {
        player_name: form.name.trim(),
        total_withdrawal: toNum(form.totalWithdraw),
      };
      if (editingUuid) {
        await updateWithdrawalDummyPlayer(editingUuid, payload);
      } else {
        await createWithdrawalDummyPlayer(payload);
      }
      router.push("/admin/leaderboards/withdrawal");
    } catch (e) {
      const data = e?.data;
      const fieldError =
        data && typeof data === "object"
          ? Object.entries(data).find(([, value]) => value !== undefined && value !== null)
          : null;
      const message = fieldError
        ? `${fieldError[0]}: ${Array.isArray(fieldError[1]) ? fieldError[1][0] : fieldError[1]}`
        : e?.message;
      setError(message ?? "Failed to save.");
    } finally {
      setSaving(false);
    }
  };

  return (
    <FormChrome
      title={editingUuid ? "Edit Dummy Data" : "Add Dummy Data"}
      onBack={() => router.push("/admin/leaderboards/withdrawal")}
      onSave={onSave}
      saving={saving}
    >
      {error && <p className="mb-4 text-sm text-red-400">{error}</p>}
      <div className="grid grid-cols-1 gap-x-8 gap-y-5 md:grid-cols-2">
        <div>
          <label className="mb-2 block text-[14px] font-semibold text-white">Player Name</label>
          <input type="text" value={form.name} onChange={set("name")} className={INPUT_BASE} />
        </div>
        <div>
          <label className="mb-2 block text-[14px] font-semibold text-white">Total Withdraw</label>
          <input
            type="number"
            min="0"
            value={form.totalWithdraw}
            onChange={set("totalWithdraw")}
            className={INPUT_BASE}
          />
        </div>
      </div>
    </FormChrome>
  );
}

export default function AddWithdrawalDummyPage() {
  return (
    <Suspense fallback={null}>
      <DummyForm />
    </Suspense>
  );
}
