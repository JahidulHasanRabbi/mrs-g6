...
import WithdrawalPlayerTable from "../../../components/admin/leaderboards/withdrawal/WithdrawalPlayerTable";
...
import {
  getWithdrawalBanners,
  archiveWithdrawalBanner,
  getWithdrawalRewardItems,
  archiveWithdrawalRewardItem,
  getWithdrawalDummyPlayers,
...
function normalizePlayer(p) {
  return {
    ...
    name: p.player ?? "",
    rank: p.rank ?? 0,
    totalWithdraw: p.total_withdraw ?? 0,
  };
}
...
export default function WithdrawalSettingsPage() {
  ...
  useEffect(() => {
    Promise.all([
      getWithdrawalBanners().catch(() => null),
      getWithdrawalRewardItems().catch(() => null),
      getWithdrawalDummyPlayers().catch(() => null),
      getLeaderboardRealRanking(2).catch(() => null),
  ...
  const confirmArchive = async () => {
    ...
    try {
      if (type === "reward") {
        await archiveWithdrawalRewardItem(item.uuid);
        setRewards((list) => list.filter((r) => r.uuid !== item.uuid));
      }
      if (type === "banner") {
        await archiveWithdrawalBanner(item.uuid);
  ...
  return (
    <div className="flex flex-col gap-6">
      <SettingsSection
        ...
        onAdd={() => router.push("/admin/leaderboards/withdrawal/banner")}
      >
        <InformationTable
          items={pagedBanners}
          onEdit={(b) => router.push(`/admin/leaderboards/withdrawal/banner?uuid=${b.uuid}`)}
        />
      ...
      <SettingsSection
        title="Rewards"
        addLabel="Add Reward"
        onAdd={() => router.push("/admin/leaderboards/withdrawal/reward")}
      >
        <DepositRewardTable
          rewards={pagedRewards}
          onEdit={(r) => router.push(`/admin/leaderboards/withdrawal/reward?uuid=${r.uuid}`)}
      ...
      <SettingsSection
        ...
        onAdd={() => router.push("/admin/leaderboards/withdrawal/dummy")}
      >
        <WithdrawalPlayerTable
          players={pagedPlayers}
          onEdit={(p) => router.push(`/admin/leaderboards/withdrawal/dummy?uuid=${p.uuid}`)}
        />
        <PaginatedFooter total={players.length} page={playerPage} setPage={setPlayerPage} />
      </SettingsSection>

      <SettingsSection title="Ranking Table">
        <RankingTable rows={pagedRankings} type="withdrawal" />
