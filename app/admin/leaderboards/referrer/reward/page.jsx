...
import {
  getReferrerRewardItem,
  createReferrerRewardItem,
  updateReferrerRewardItem,
} from "../../../../api/adminApi";
...
function RewardForm() {
  ...
  useEffect(() => {
    ...
    getReferrerRewardItem(editingUuid)
      .then((r) => {
        setForm({
          name: r.reward_name ?? "",
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editingUuid) {
        await updateReferrerRewardItem(editingUuid, payload);
      } else {
        await createReferrerRewardItem(payload);
      }
...
export default function AddReferrerRewardPage() {
  return (
