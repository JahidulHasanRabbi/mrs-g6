...
import {
  getDepositRewardItem,
  createDepositRewardItem,
  updateDepositRewardItem,
} from "../../../../api/adminApi";

const ITEM_TYPES = [
  { value: 1, label: "Top deposit" },
];
...
function RewardForm() {
  ...
  useEffect(() => {
    ...
    getDepositRewardItem(editingUuid)
      .then((r) => {
        setForm({
          name: r.reward_name ?? "",
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editingUuid) {
        await updateDepositRewardItem(editingUuid, payload);
      } else {
        await createDepositRewardItem(payload);
      }
...
export default function AddDepositRewardPage() {
  return (
