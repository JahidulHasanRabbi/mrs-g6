...
import {
  getTurnoverDummyPlayer,
  getTurnoverDummyPlayers,
  createTurnoverDummyPlayer,
  updateTurnoverDummyPlayer,
} from "../../../../api/adminApi";
...
function DummyForm() {
  ...
  useEffect(() => {
    if (editingUuid) {
      getTurnoverDummyPlayer(editingUuid)
        .then((d) => {
          setForm({
            name: d.player ?? "",
            totalTurnover: String(d.total_turnover ?? ""),
          });
  ...
  const onSave = async () => {
    ...
    try {
      const payload = {
        ...
        total_turnover: toNum(form.totalTurnover),
      };
      if (editingUuid) {
        await updateTurnoverDummyPlayer(editingUuid, payload);
      } else {
        await createTurnoverDummyPlayer(payload);
      }
      router.push("/admin/leaderboards/turnover");
    } catch (e) {
      const data = e?.data;
      const fieldError =
  ...
  return (
    <FormChrome
      ...
      onBack={() => router.push("/admin/leaderboards/turnover")}
      onSave={onSave}
      saving={saving}
    >
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-5 md:grid-cols-2">
        ...
        <div>
          <label className="mb-2 block text-[14px] font-semibold text-white">Total Turnover</label>
          <input
            type="number"
            min="0"
            value={form.totalTurnover}
            onChange={set("totalTurnover")}
...
export default function AddTurnoverDummyPage() {
  return (
