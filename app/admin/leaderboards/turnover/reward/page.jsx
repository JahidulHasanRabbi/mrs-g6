...
import {
  getTurnoverRewardItem,
  createTurnoverRewardItem,
  updateTurnoverRewardItem,
} from "../../../../api/adminApi";
...
function RewardForm() {
  ...
  useEffect(() => {
    ...
    getTurnoverRewardItem(editingUuid)
      .then((r) => {
        setForm({
          name: r.reward_name ?? "",
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // Never send image: null — the API 400s on an explicit null; omit the
      // key entirely unless the admin actually picked a new file.
      if (form.imageFile) {
        payload.image = form.imageFile;
      }
      if (editingUuid) {
        await updateTurnoverRewardItem(editingUuid, payload);
      } else {
        await createTurnoverRewardItem(payload);
      }
...
export default function AddTurnoverRewardPage() {
  return (
