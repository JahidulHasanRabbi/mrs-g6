...
// TODO (Backend): Replace mock data with real API — fields: phone_number, username, station, created_at, category, details, reward_name
    ...
    <div className="min-h-screen bg-[#07190d]">
      ...
      <main className="min-h-screen pl-[388px] pr-10 pt-10 pb-10">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-400">View reward redemption history across all members</p>
        </div>

        {/* Filters — TODO (Backend): wire up filter params */}
        <div className="mb-6 flex flex-wrap gap-3">
          {["Date/Time", "Category", "Reward Details", "Reward Name", "Username", "Phone Number", "Station"].map((f) => (
            <input
              key={f}
              type="text"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  {["Phone Number", "Username", "Station", "Date/Time", "Category", "Reward Details", "Reward Name"].map((h) => (
              ...
              <tbody>
                {MOCK_REWARD_REPORT.map((row) => (
                  <tr key={row.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="px-5 py-3 text-sm font-bold text-[#e9af41]">{row.reward_name}</td>
                  </tr>
                ))}
              </tbody>
            </table>
