...
export default function WithdrawalPlayerTable({ players = [], onEdit }) {
  return (
        ...
        <table className="w-full min-w-[700px]">
          <thead>
            <tr style={{ backgroundImage: HEADER_BG }} className="text-left">
              ...
              <th className="px-5 py-4 text-[13px] font-semibold text-[#fbeed2]">Total Withdraw</th>
            ...
            {players.length === 0 ? (
              ...
              players.map((p) => (
                <tr key={p.id} className="border-b border-white/5 align-middle last:border-b-0 hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-5 text-[12px] text-white">{fmt(p.totalWithdraw)}</td>
