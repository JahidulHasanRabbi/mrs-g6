...
// Referrer leaderboard dummy-player table. Referral fake rows track both the
// qualifying deposit amount used for ranking and the qualified member count.
...
export default function ReferrerPlayerTable({ players = [], onEdit }) {
  return (
        ...
        <table className="w-full min-w-[820px]">
          <thead>
            <tr style={{ backgroundImage: HEADER_BG }} className="text-left">
              ...
              <th className="px-5 py-4 text-[13px] font-semibold text-[#fbeed2]">Referral Deposit</th>
              <th className="px-5 py-4 text-[13px] font-semibold text-[#fbeed2]">Total Member</th>
            ...
            {players.length === 0 ? (
              ...
              players.map((p) => (
                <tr key={p.id} className="border-b border-white/5 align-middle last:border-b-0 hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-5 text-[12px] text-white">{fmt(p.referralDeposit)}</td>
                  <td className="px-5 py-5 text-[12px] text-white">{fmt(p.totalMember)}</td>
                  <td className="px-5 py-5">
                    <RowActions onEdit={() => onEdit?.(p)} showArchive={false} />
                  </td>
                </tr>
              ))
            )}
