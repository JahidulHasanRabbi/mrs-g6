...
const SpinButton = memo(function SpinButton({ spins, tokens, onClick, image, className = "w-[160px] h-[70px]", disabled = false, dimmed = false }) {
  return (
    <motion.div 
      className={`relative ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} ${className}`}
      onClick={disabled ? undefined : onClick}
      initial={{ opacity: 0, scale: 0.8, y: 20 }}
      animate={{ opacity: disabled ? 0.5 : dimmed ? 0.45 : 1, scale: 1, y: 0 }}
