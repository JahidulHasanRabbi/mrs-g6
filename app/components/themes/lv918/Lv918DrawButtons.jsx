...
/**
 * Lv918 Draw buttons — same 10 / 50 / 100 options + token cost as the shared
 * DrawButtons, restyled to the theme: deep magenta/plum plaques with a gold
 * edge, and the featured (50) plaque in gold. Text is gold/cream (or dark on gold).
 */
function DrawButton({ draws, tokens, featured, onClick, disabled }) {
  const bg = featured
    ? "linear-gradient(to bottom, #ffe6a3, #d99a1e)"
    : "linear-gradient(to bottom, #b81f6e, #4a0a2e)";
  const border = featured ? "#8a5e0e" : "#2a0518";
  ...
  const mainColor = featured ? "#3a2a05" : LV918_COLORS.goldBright;
  const tokenColor = featured ? "rgba(58,42,5,0.85)" : LV918_COLORS.creamMuted;
...
export default function Lv918DrawButtons({ onDraw, disabled, tokensPerRound = 10 }) {
  return (
