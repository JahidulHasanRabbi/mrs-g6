...
import KgameDrawButtons from './KgameDrawButtons';
import KgamePrizeList from './KgamePrizeList';
import KgameWinnerList from './KgameWinnerList';
import KgameTerms from './KgameTerms';
...
const WELCOME_SEEN_KEY = 'mrs_kgame99_egg_welcome_seen';

/**
 * Kgame99 Smash Egg (Figma nodes 154:367 loading / 154:289 welcome / 154:243
 ...
 */
export default function Kgame99SmashEggPage() {
  const {
    isCracked,
    isProcessing,
    isModalOpen,
    ...
    return (
      <div className="relative min-h-screen w-full overflow-hidden bg-black">
        <div className="fixed inset-0 left-1/2 -translate-x-1/2 w-full max-w-[475px]">
          <Image src={KGAME99_ASSETS.egg.bgLoading} alt="" fill priority className="object-cover" sizes="475px" />
        </div>
        <div className="relative z-10 flex flex-col items-center gap-6 pt-[240px] px-6">
          ...
          >
            <img src={KGAME99_ASSETS.egg.logo} alt="Smash Egg" draggable={false} className="absolute inset-0 h-full w-full select-none object-contain" />
          </motion.div>
          <div className="w-full max-w-[342px] rounded-[16px] border border-[rgba(255,246,223,0.15)] bg-[rgba(20,32,54,0.7)] backdrop-blur-[6px] p-[9px]">
            <div
              ...
              style={{ width: `${bootProgress}%`, backgroundColor: KGAME99_COLORS.goldBright }}
            />
          </div>
        </div>
      </div>
  ...
  return (
    <KgameShell bg={KGAME99_ASSETS.egg.bg} onInfoClick={openHistory} title="SMASH EGG" titleIcon={SMASH_EGG_ASSETS.headerEggIcon}>
      <div className="relative flex flex-col items-center px-4">
        ...
        >
          <AnimatePresence mode="popLayout">
            ...
            >
              <img
                src={isCracked ? KGAME99_ASSETS.egg.eggCracked : KGAME99_ASSETS.egg.eggIntact}
                alt="Crystal egg"
                draggable={false}
        ...
        <div className="relative z-10 mt-6 w-full max-w-[390px]">
          <KgameDrawButtons onDraw={handleDraw} disabled={isProcessing || !gameEnabled} tokensPerRound={tokensPerRound} />
        </div>

        {/* Prize list / winner feed / T&C — kgame99-themed (ornate panel +
            gold headings), same data as the shared cards. */}
        <div className="relative z-10 mt-8 flex w-full flex-col items-center gap-6 pb-4">
          <KgamePrizeList prizes={rewardBoard.prizes} creditRanges={rewardBoard.creditRanges} />
          <KgameWinnerList winners={winningHistory} />
          <KgameTerms termsText={termsText} />
        </div>
      </div>
      ...
      <KgameDialog open={showWelcome} onClose={dismissWelcome} frameless>
        <KgameOrnateCard>
          <div className="flex items-center gap-2">
            <img src={KGAME99_ASSETS.ui.iconGift} alt="" className="w-6 h-6" />
            <p className="text-[20px] tracking-[2px] uppercase" style={{ fontFamily: 'var(--font-acme), sans-serif', color: KGAME99_COLORS.cream }}>
              Welcome!
            </p>
          </div>
          <div className="mt-3 flex items-center justify-center gap-2 rounded-[8px] ring-1 ring-inset ring-[rgba(242,203,122,0.3)] bg-[rgba(8,20,44,0.5)] px-4 py-3 w-full">
            <img src={KGAME99_ASSETS.ui.iconCoins} alt="" className="w-[26px] h-[26px] shrink-0" />
            <div className="flex flex-col items-center">
              <p className="text-[12px] uppercase" style={{ color: '#ffb77d', fontFamily: 'var(--font-acme), sans-serif' }}>
                You have received
              </p>
              <p className="text-[11px] mt-1" style={{ color: KGAME99_COLORS.sand, fontFamily: 'var(--font-rubik), sans-serif' }}>
                {tokenBalance} Free Tokens
              </p>
            </div>
          </div>
        </KgameOrnateCard>
        <KgameButton onClick={dismissWelcome}>Start Playing</KgameButton>
      </KgameDialog>
      ...
      {/* Result modal — same content + actions as the default portal (You Won
          ...
          Kgame99 ornate popup frame. */}
      {/* Use the transparent ornate frame — kgame99-popup.png was flattened to
          ...
          use the themed KgameButton instead of the plain gold/dark pills. */}
      <ThemedResultModal
        isOpen={isModalOpen}
        onClose={closeModal}
        onReturn={handleReturnToWebsite}
        prize={wonPrize}
        frameBg={KGAME99_ASSETS.ui.dialogFrameTall}
