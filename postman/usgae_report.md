...
* The **Summary** endpoint is **flat** — top-level scalar fields only, no pagination.
...
One row per game (or just the selected game), with usage, the full **reward breakdown** (money,
tokens, prizes, World Cup score given out), and the new-vs-existing user split. Sort the rows by
`unique_players` descending to get the Top Mini Game ranking (Report 5).

Reward fields are anchored to the **play** (the play's date and the item it awarded). `credit_rm`
is the actual money won; `tokens_awarded` / `wc_score_awarded` are valued from the awarded item's
configured amount; `prizes_awarded` is a count. **Prediction** has no per-play item (it's free and
pays out on streak milestones), so all four of its reward fields are `0`/`null`.
...
| tokens\_awarded | Int | Tokens **given out** as a reward (token-type plays, valued at the item's token amount) |
| prizes\_awarded | Int | Count of plays that awarded a physical prize / item |
| wc\_score\_awarded | Int / null | World Cup score points awarded (Penalty Kick only; `null` for games with no score reward) |
